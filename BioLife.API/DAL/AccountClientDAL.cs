   ...
   public class AccountClientDAL : GenericService<AccountClient>
    {
        ...
        public int CreateAccountClient(AccountClient model)
        {
            try
            {
                ...
                return _DbWorker.ExecuteNonQuery(SPName.sp_InsertAccountClient, objParam);
            }
            catch (Exception ex)
            {
                return 0;
        ...
        public AccountClient GetByUsername(string user_name)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    ...
                    var main_account = _DbContext.AccountClients.FirstOrDefault(x => x.UserName == user_name);
                    if (main_account != null)
                    {
                        return main_account;
                    }
        ...
        public async Task<int> UpdataAccountClient(AccountClient model)
        {
            try
            {
                ...
                return _DbWorker.ExecuteNonQuery(SPName.sp_UpdateAccountClient, objParam);    
            }
            catch (Exception ex)
            {
        ...
        public AccountClient GetById(long id)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    ...
                    var main_account = _DbContext.AccountClients.FirstOrDefault(x => x.Id == id);
                    if (main_account != null)
                    {
                        return main_account;
                    }
